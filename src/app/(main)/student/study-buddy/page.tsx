...
import { studyBuddyChatbot } from '@/ai/flows/ai-study-buddy';
...
export default function StudyBuddyPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'bot',
      content: "Hi! I'm your AI Study Buddy. What concept can I help you understand today?",
    },
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await studyBuddyChatbot({
        concept: input,
      });
      const botMessage: Message = { role: 'bot', content: response.explanation };
      ...
      <Card className="flex-1 flex flex-col">
        <CardHeader className="flex flex-row items-center">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <p className="text-lg font-medium leading-none">AI Study Buddy</p>
              <p className="text-sm text-muted-foreground">Instant academic concept explanations</p>
            </div>
          </div>
        </CardHeader>
